 ...
 *
 * Shared by the `db:seed` CLI (prisma/seed.ts) and the one-time setup route
 * (src/app/api/setup/route.ts), so both create exactly the same data.
 *
 * Every step is idempotent: existing records are left alone and reported as
 * skipped, so a second run cannot duplicate a user or reissue a password.
 *
 * Temporary passwords are generated fresh, returned to the caller, and never
 * written to a file or stored anywhere (Section 4's explicit instruction).
 * Every seeded user has mustResetPassword = true, so each one is single-use.
 */
...
export type SeedCredential = { email: string; password: string };
export type SeedResult = { log: string[]; credentials: SeedCredential[] };
...
export async function runSeed(prisma: PrismaClient): Promise<SeedResult> {
  ...
  if (!existingCompany) {
    ...
    log("Company record created.");
  } else {
    log("Company record already exists — skipped.");
  }
  ...
  log(`Seeded ${roleIdByName.size} roles.`);
  ...
  for (const person of people) {
    ...
    if (existingUser) {
      log(`User ${person.email} already exists — skipped.`);
      continue;
    }
    ...
    log(`Seeded user ${person.email} (${person.roles.join(", ")}).`);
  }

  return { log: lines, credentials: credentialsToPrint };
}
